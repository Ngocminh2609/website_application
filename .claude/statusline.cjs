 ...
 * Rendering is config-driven via statuslineLayout in .fisrc.json.
...
const { loadConfig, readSessionState } = require('./hooks/lib/fis-config-utils.cjs');
...
// Read stdin with optional inactivity timeout (FIS_STATUSLINE_STDIN_TIMEOUT_MS)
async function readStdin() {
  return new Promise((resolve, reject) => {
    const chunks = [];
    stdin.setEncoding('utf8');
    const parsedTimeout = Number.parseInt(env.FIS_STATUSLINE_STDIN_TIMEOUT_MS || '', 10);
...
async function main() {
  try {
    ...
    if (sessionId && contextSize > 0) {
      try {
        const contextDataPath = path.join(os.tmpdir(), `fis-context-${sessionId}.json`);
