...
Generates three views keyed by directory identity:
  1. Standard active (31): tier1 + tier2 from skill-tiers.json
  2. Inactive optional packs (71): per-pack grouping from packs section
  3. Full canonical inventory (102): all skills
...
def load_skill_tiers(repo_root: Path) -> dict:
    ...
    if len(set(standard)) != 31:
        raise SystemExit(
            f"Error: skill-tiers.json standard catalog (tier1+tier2) must have 31 unique IDs; got {len(set(standard))}"
        )
    optional = manifest.get("optional", [])
    if len(optional) != 71:
        raise SystemExit(
            f"Error: skill-tiers.json optional must have 71 IDs; got {len(optional)}"
        )
...
def get_standard_ids(manifest: dict) -> list[str]:
    """Return ordered list of standard 31 skill directory IDs."""
    tier1 = manifest.get("tiers", {}).get("tier1", [])
    tier2 = manifest.get("tiers", {}).get("tier2", [])
    return tier1 + tier2
...
def get_optional_ids(manifest: dict) -> list[str]:
    """Return optional 71 skill directory IDs (flat)."""
...
def validate_catalog_inputs(skills: list[dict], manifest: dict, base_path: Path) -> None:
    """Require an exact 102-skill on-disk inventory matching source descriptors."""
    ...
    if len(skills) != 102:
        diagnostics.append(f"expected exactly 102 canonical SKILL.md files; found {len(skills)}")
    ...
    if len(standard_ids) != 31 or len(set(standard_ids)) != 31:
        diagnostics.append("standard tiers must contain 31 unique IDs")
    if len(optional_ids) != 71 or len(set(optional_ids)) != 71:
        diagnostics.append("optional must contain 71 unique IDs")
    ...
    if len(pack_members) != 71 or set(pack_members) != set(optional_ids):
...
def write_catalog_yaml(
    ...
    """Write SKILLS.yaml with three views: standard-31, optional-packs-71, full-102."""
    skill_index = index_by_id(skills)
    standard_ids = get_standard_ids(manifest)
    optional_ids = get_optional_ids(manifest)
    ...
    lines = [
        ...
        "",
        "# ── Standard Active Skills (31: tier1 + tier2) ────────────────────────────",
        "standard_active:",
    ]
    ...
    lines.append("# ── Inactive Optional Packs (71 skills in packs) ─────────────────────────")
    ...
    lines.append("# ── Full Canonical Inventory (102 skills) ────────────────────────────────")
...
def write_catalog_markdown(
    ...
    """Write SKILLS.md with three sections: standard-31, optional-packs-71, full-102."""
    skill_index = index_by_id(skills)
    standard_ids = get_standard_ids(manifest)
    optional_ids = get_optional_ids(manifest)
    packs = manifest.get("packs", {})
    ...
    lines = [
        ...
        "## Contents", "",
        "- [Standard Active Skills (31)](#standard-active-skills-31)",
        "- [Inactive Optional Packs (71)](#inactive-optional-packs-71)",
        "- [Full Canonical Inventory (102)](#full-canonical-inventory-102)",
        "",
    ]

    # ── Section 1: Standard Active (31)
    tier1 = manifest.get("tiers", {}).get("tier1", [])
    tier2 = manifest.get("tiers", {}).get("tier2", [])
    tier1_set = set(tier1)

    lines.extend([
        "## Standard Active Skills (31)",
        "",
        "These 31 skills are active by default. Use `/<id>` to invoke directly.",
        "",
        "### Tier 1 — Core (10)",
        "",
    ])
    ...
    # ── Section 2: Optional Packs (71)
    lines.extend([
        "## Inactive Optional Packs (71)",
        "",
        "These 71 skills are inactive by default. Enable a pack via DAI Kit Detail → Optional skill packs.",
    ...
    for pack_name, pack_info in packs.items():
        ...
        for skill in pack_skills:
            ...
            desc = str(skill["description"]).replace("\n", " ").strip()
            lines.extend([
                f"- {icons}`/{skill['id']}` — display `{skill['display_name']}`: {desc[:100].rstrip()}",
            ])
        lines.append("")

    # ── Section 3: Full Inventory (102)
    lines.extend([
        "## Full Canonical Inventory (102)",
        "",
        "All skills ordered as: standard-31 then optional-71. "
    ...
    for skill in all_skills:
        ...
        lines.append(
            f"| `/{skill['id']}` ({active}) | `{skill['display_name']}` "
...
def main() -> None:
    ...
    print(f"  Standard active: {len(standard_found)}/31")
    print(f"  Optional:        {len(optional_found)}/71")
    print(f"  Total:           {len(standard_found) + len(optional_found)}/102")
