...
test('catalog metadata reports 31 standard, 71 optional, and 102 total', () => {
  assertMatch(guideYaml, /standard_count: 31\b/, 'standard_count must be 31');
  assertMatch(guideYaml, /optional_count: 71\b/, 'optional_count must be 71');
  assertMatch(guideYaml, /total_count: 102\b/, 'total_count must be 102');
  assert(
    !/\*\*Total(?: Skills)?\*\*:\s*31\b/.test(guideMarkdown),
    'markdown catalog must not claim that 31 is the total inventory',
  );
});

test('catalog emits exact 31/71/102 view sizes', () => {
  ...
  assert((standard.match(/^\s+- id:/gm) || []).length === 31, 'standard_active must contain 31 skills');
  assert((optional.match(/^\s+- id:/gm) || []).length === 71, 'optional_packs must contain 71 skills');
  assert((full.match(/^\s+- id:/gm) || []).length === 102, 'full_inventory must contain 102 skills');
});
