 ...
 * Bypass via env FIS_SIMPLIFY_DISABLED=1 or .fisrc.json hooks.simplify-gate=false.
 */

const { execFileSync } = require('node:child_process');
const fs = require('node:fs');
const path = require('node:path');
const { isHookEnabled } = require('./lib/fis-config-utils.cjs');
...
function loadConfig(cwd) {
  // Read .claude/.fisrc.json (canonical) or .fisrc.json (project root) scoped to cwd.
  let user = {};
