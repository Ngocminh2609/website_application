#!/usr/bin/env node
/**
 * Plan Subagent Stop Hook - Craft Skill Reminder
 *
 * Fires when Plan subagent completes. Reminds to invoke /fis-craft --auto before implementation.
...
try {
  ...
  const { isHookEnabled, readSessionState } = require('./lib/fis-config-utils.cjs');
  ...
  try {
    ...
    console.log('MUST invoke /fis-craft --auto skill before implementing the plan');
    if (planPath) {
      const planMdPath = path.join(planPath, 'plan.md');
      console.log(`Best Practice: Run /clear then /fis-craft ${planMdPath}`);
    } else {
      // Fallback when plan path unavailable
      console.log('Best Practice: Run /clear then /fis-craft {full-absolute-path-to-plan.md}');
    }
