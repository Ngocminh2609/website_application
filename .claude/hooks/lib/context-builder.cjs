...
const USAGE_CACHE_FILE = path.join(os.tmpdir(), 'fis-usage-limits-cache.json');
...
function buildContextSection(sessionId) {
  ...
  try {
    const contextPath = path.join(os.tmpdir(), `fis-context-${sessionId}.json`);
 ...
 * @param {Object} [params.config] - FIS config (auto-loaded if not provided)
