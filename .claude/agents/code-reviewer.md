...
When performing pre-landing review (from `/fis-ship` or explicit checklist request), load and apply checklists from `fis-code-review/references/checklists/` using the workflow in `fis-code-review/references/checklist-workflow.md`. Two-pass model: critical (blocking) + informational (non-blocking).
...
Use `/fis-scout` with edge-case-focused prompt:
```
