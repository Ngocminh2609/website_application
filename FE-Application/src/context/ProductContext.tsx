...
export const ProductProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const fetchingHomePromise = useRef<Promise<
    [Product[], Product[], Category[]]
  > | null>(null);
  const fetchingBrandPromises = useRef<Map<string, Promise<Product[]>>>(
    new Map(),
  );
  ...
  const initializeHomeData = useCallback(
    async (force = false) => {
      ...
      if (fetchingHomePromise.current) {
        await fetchingHomePromise.current;
        return;
      }
