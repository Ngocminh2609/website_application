    ...
    const fetchStats = async (selectedPeriod: string) => {
        ...
        try {
            ...
            switch (selectedPeriod) {
                case 'daily': res = await statisticsApi.getDailyStats(); break;
                case 'weekly': res = await statisticsApi.getWeeklyStats(); break;
                case 'monthly': res = await statisticsApi.getMonthlyStats(); break;
                case 'yearly': res = await statisticsApi.getYearlyStats(); break;
            }
    ...
    return (
        <div style={{ marginTop: 20 }}>
            ...
            {loading ? (
                ...
                <Empty description={<Text style={{ color: 'var(--text-muted)' }}>Chưa có dữ liệu thống kê cho kỳ này</Text>} />
            ) : (
                <Row gutter={[24, 24]}>
                    {/* BIỂU ĐỒ DOANH THU */}
                    <Col span={24}>
                            ...
                            <div style={{ width: '100%', height: 350 }}>
                                <ResponsiveContainer>
                                    <BarChart data={data}>
                                        ...
                                        />
                                        <Bar dataKey="orderCount" fill="#a855f7" radius={[4, 4, 0, 0]} barSize={40} />
                                    </BarChart>
                                </ResponsiveContainer>
                            </div>
                        </Card>
