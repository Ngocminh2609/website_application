    ...
    return (
        <div className="main-content">
            <Breadcrumb style={{ marginBottom: '24px' }}>
                ...
                <Breadcrumb.Item><span style={{ color: 'var(--text-main)' }}>Danh sách yêu thích</span></Breadcrumb.Item>
            </Breadcrumb>

            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '40px' }}>
                <HeartFilled style={{ fontSize: '36px', color: '#f43f5e' }} />
                <Title level={1} style={{ margin: 0, color: 'var(--text-main)' }}>Danh sách yêu thích</Title>
                <Text style={{ fontSize: '18px', marginLeft: 'auto', color: 'var(--text-muted)' }}>
            ...
            {wishlistItems.length === 0 ? (
                    ...
                    <Empty
                        ...
                        description={
                            <div style={{ marginTop: '24px' }}>
                                <Text style={{ fontSize: '20px', display: 'block', marginBottom: '12px', color: 'var(--text-main)', fontWeight: 600 }}>
                                    Danh sách yêu thích hiện đang trống
                                </Text>
                                <Text style={{ color: 'var(--text-muted)', fontSize: '16px' }}>
                                    Hãy thêm những sản phẩm bạn yêu thích để dễ dàng mua sắm sau này.
                                </Text>
                            </div>
                        }
                    >
                        <Link to="/products">
                            <Button type="primary" size="large" style={{ marginTop: '24px', borderRadius: '8px' }}>
                                Tiếp tục mua sắm
                            </Button>
                        </Link>
                    </Empty>
                ...
                <Row gutter={[32, 40]}>
                    {wishlistItems.map((product: Product) => (
                        <Col xs={24} sm={12} md={8} lg={6} xl={4} className="grid-5-cols-col" key={product.id}>
