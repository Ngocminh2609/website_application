...
import { DeleteOutlined, ShoppingCartOutlined, CreditCardOutlined, EnvironmentOutlined, PhoneOutlined, TagOutlined, CheckCircleOutlined, CloseCircleOutlined, PlusOutlined, HomeOutlined, CheckOutlined } from '@ant-design/icons';
...
const CartPage: React.FC = () => {
    ...
    const handleAddAddress = async (values: Omit<UserAddress, 'id' | 'isDefault'>) => {
        try {
    ...
    const handleProvinceChange = async (provinceCode: number, option: { label: string; value: string | number } | { label: string; value: string | number }[] | undefined) => {
            ...
            <Modal
                title={
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        ...
                        <Title level={4} style={{ margin: 0 }}>Thông tin nhận hàng</Title>
                    </div>
                }
                open={isModalVisible}
                onCancel={() => {
                ...
                {addressLoading ? (
                    ...
                    >
                        ...
                        <Row gutter={16}>
                            <Col span={12}>
                                ...
                                <Form.Item name="province" noStyle><input type="hidden" /></Form.Item>
                            </Col>
                            <Col span={12}>
                                <Form.Item
                                    name="ward"
                    ...
                    <div style={{ marginTop: 20 }}>
                        ...
                        >
                            <Space direction="vertical" style={{ width: '100%' }} size="middle">
                                {addresses.map(addr => (
                                    ...
                                    >
                                        <div style={{ display: 'inline-block', marginLeft: '8px', verticalAlign: 'top', width: '90%' }}>
                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                                <Text strong style={{ fontSize: '16px' }}>{addr.fullName}</Text>
                                                {addr.isDefault && <Tag color="blue" style={{ borderRadius: '4px', margin: 0 }}>Mặc định</Tag>}
                                            </div>
                                            <div style={{ marginTop: '4px' }}>
                                                <Text type="secondary"><PhoneOutlined /> {addr.phoneNumber}</Text>
                                            </div>
                                            <div style={{ marginTop: '4px' }}>
                                                <Text style={{ color: 'var(--text-main)' }}>
                                                    <EnvironmentOutlined /> {addr.detailAddress}, {addr.ward}, {addr.province}
                                                </Text>
                                            </div>
                                        </div>
                                    </Radio>
                        ...
                        <div style={{ marginBottom: 24 }}>
                            <Text strong style={{ display: 'block', marginBottom: 12 }}>Hình thức thanh toán:</Text>
                            <Radio.Group
                                ...
                                <Space direction="vertical" style={{ width: '100%' }}>
                                    <Radio
                                        ...
                                        style={{
                                            ...
                                            borderRadius: '8px',
                                            margin: 0,
                                            background: paymentMethod === 'VNPAY' ? 'rgba(99, 102, 241, 0.05)' : 'transparent'
                                        }}
                                    >
                                        <Space>
                                            <img src={vnpayLogo} alt="VNPay" style={{ height: 20 }} />
                                            <Text>Thanh toán qua VNPay (ATM/Internet Banking/QR Code)</Text>
                                        </Space>
                                    </Radio>
                                    <Radio
                                        value="COD"
                                    ...
                                    >
                                        <Space>
                                            <CreditCardOutlined style={{ color: 'var(--primary-color)', fontSize: 18 }} />
                        ...
                        <BaseButton
                            type="primary"
                            style={{ width: '100%', height: 50, fontSize: '1.1rem', borderRadius: '12px' }}
                            loading={checkoutLoading}
                            onClick={handleConfirmPayment}
                            icon={<CheckOutlined />}
                        >
                            {paymentMethod === 'VNPAY' ? 'Tiếp tục thanh toán VNPay' : 'Xác nhận đặt hàng'}
                        </BaseButton>
                    </div>
                )}
            </Modal>
