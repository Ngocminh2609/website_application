...
export const productApi = {
    ...
    searchProducts: (query: string): Promise<Product[]> =>
        apiClient.fetch<Product[]>(`${BASE_PATH}/search?query=${encodeURIComponent(query)}`),
    ...
    getProductsByBrand: (brand: string): Promise<Product[]> =>
        apiClient.fetch<Product[]>(`${BASE_PATH}/brand/${encodeURIComponent(brand)}`),
     ...
     */
    updateProduct: (id: number, data: Partial<ProductRequest>): Promise<Product> =>
        apiClient.fetch<Product>(`${BASE_PATH}/${id}`, {
            method: 'PUT',
            body: JSON.stringify(data),
        }),
